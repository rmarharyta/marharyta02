...
df["Lenght"] = df["Lenght"].apply(
    lambda x: round(x, 2) if x % 1 != 0 else int(x))
array_data = np.column_stack((df["Vector"].to_numpy(), df["CoordinateX"].to_numpy(
), df["CoordinateY"].to_numpy(), df["Lenght"].to_numpy()))
